...
@app.route("/")
def f_compare_compare():
    return render_template('/201810370311023/compare.html', )


@app.route('/1/pred_comp', methods=['POST'])
def f_201810370311023_predict_compare():
    ...
    model_dict = {'CNNModel'     :   'static/MLModule/modelCNN.h5',
                  'MobileModel'  :   'static/MLModule/modelmobile_1(3).h5',
                  'Resnet50Mod'  :   'static/MLModule/Resnet_1.h5',
                  }
    ...
    return f_201810370311023_predict_result_compare(respon_model, chosen_model, running_time, filename[7:])

@app.route('/1/pred_comps', methods=['POST'])
def f_201810370311023_predicts_compare():
    ...
    return f_201810370311023_predict_result_compare(respon_model, chosen_model, running_time, 'temp.jpg')

def f_201810370311023_predict_result_compare(probs, mdl, run_time, img):
    ...
    return render_template('/201810370311023/result_compare.html', labels=labels, 
                            probs=probs, mdl=mdl, run_time=run_time, pred=idx_pred, img=img)

# Select Model f_201810370311023 Brain Tumor Disease
@app.route('/1/select')
def f_201810370311023_select():
    return render_template('/201810370311023/select.html', )

@app.route('/1/pred_select', methods=['POST'])
def f_201810370311023_predict_select():
    ...
    return f_201810370311023_predict_result_select(chosen_model, runtimes, respon_model, filename[7:])

@app.route('/1/pred_selects', methods=['POST'])
def f_201810370311023_predicts_select():
    ...
    return f_201810370311023_predict_result_select(chosen_model, runtimes, respon_model, 'temp.jpg')

def f_201810370311023_predict_result_select(model, run_time, probs, img):
    class_list = {'No DR': 0, 'Mild': 1, 'Moderate': 2,'Severe': 3, 'Proliferative DR' : 4}
    idx_pred = probs.index(max(probs))
    labels = list(class_list.keys())
    return render_template('/201810370311023/result_select.html', labels=labels, 
